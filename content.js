...
const PLATFORM_PRESETS = {
  chatgpt: {
    ...
    rules: [
      {
        listSelector: "main",
        itemSelector: 'section[data-testid^="conversation-turn-"][data-turn][data-turn-id]',
        userSelector: 'section[data-testid^="conversation-turn-"][data-turn="user"][data-turn-id]',
        assistantSelector: 'section[data-testid^="conversation-turn-"][data-turn="assistant"][data-turn-id]',
        presetName: "conversation-turn-section",
        groupingStrategy: "chatgpt-turn-group",
      },
    ],
  },
...
function debounceApply() {
  if (lastApplyTimer) clearTimeout(lastApplyTimer);
  lastApplyTimer = setTimeout(() => {
    if (currentRule?.autoTrim) applyRule(currentRule);
  }, 300);
}
...
function getSimpleSelector(el) {
  ...
  for (const attr of stableAttrs) {
    ...
    if (value) return `${tag}[${attr}="${cssEscapeSafe(value)}"]`;
  }
...
function getSharedItemSelector(el, listEl) {
  if (!(el instanceof Element)) return "";
  if (!(listEl instanceof Element) || !listEl.contains(el)) return getUniqueSelector(el);
  ...
  if (stableClasses.length) return `${tag}.${cssEscapeSafe(stableClasses[0])}`;

  if (el.parentElement === listEl) return `:scope > ${tag}`;
  return tag;
}
...
function ensureBootstrapObserver() {
  ...
  bootstrapObserver = new MutationObserver(() => {
    ...
    if (result?.ok && result?.detected) stopBootstrapObserver();
  });
...
function inferMessageRole(el) {
  ...
  const directTurn = (el.getAttribute("data-turn") || "").toLowerCase();
  if (directTurn === "user") return "user";
  if (directTurn === "assistant") return "assistant";

  const directAuthor = (el.getAttribute("data-message-author-role") || "").toLowerCase();
  if (directAuthor === "user") return "user";
  if (directAuthor === "assistant") return "assistant";
  ...
  if (/(^|[^a-z])(assistant|ai|bot|answer|reply|model|doubao|chatgpt|claude|gemini)([^a-z]|$)/.test(fields)) return "assistant";
...
function elementLooksLikeRoundBlock(el) {
  if (!(el instanceof Element)) return false;
  const roleNodes = Array.from(el.querySelectorAll("[data-message-author-role],[data-testid],[data-test],[aria-label],[role],[data-turn]"));
  let userCount = 0;
...
function buildChatGPTTurnGroups(items) {
  const groups = [];
  let currentGroup = [];

  for (const item of items) {
    const role = (item.getAttribute("data-turn") || inferMessageRole(item)).toLowerCase();
    ...
    if (role === "assistant") {
      if (!currentGroup.length) currentGroup = [item];
      else currentGroup.push(item);
      continue;
    }

    if (currentGroup.length) currentGroup.push(item);
    else groups.push([item]);
  }

  if (currentGroup.length) groups.push(currentGroup);

  return { mode: "chatgpt-turn-grouped", groups };
}

function buildRoundGroups(items, rule = null) {
  if (!items.length) {
    return { mode: "single", groups: [] };
  }

  if (
      rule?.groupingStrategy === "chatgpt-turn-group" ||
      rule?.groupingStrategy === "chatgpt-turn-pair"
  ) {
    return buildChatGPTTurnGroups(items);
  }
  ...
  if (userCount && assistantCount) {
    ...
    for (let i = 0; i < items.length; i += 1) {
      ...
      if (role === "assistant") {
        if (!current.length) {
          ...
          if (prev && prevRole !== "assistant") current = [prev];
        }
        current.push(item);
        groups.push(current);
        current = [];
    ...
    if (current.length) groups.push(current);
    if (groups.length) return { mode: "role-grouped", groups };
  }
...
function getMatchedItems(rule) {
  if (!rule?.listSelector || !rule?.itemSelector) return { listEl: null, items: [] };
  ...
  if (!listEl) throw new Error(`未找到对话容器：${rule.listSelector}`);
  ...
  items = items.filter(
      (el) => el instanceof HTMLElement && isElementVisible(el) && !el.hasAttribute("data-llm-dom-trimmer-skip")
  );
...
function cleanupChatGptOrphans(rule, grouping) {
  if (detectPlatform().key !== "chatgpt") return;
  if (!grouping?.groups?.length) return;

  const keptGroups = grouping.groups.slice(-Math.max(1, Number.parseInt(rule.keepCount || 5, 10)));
  const firstKept = keptGroups[0]?.[0];
  if (!(firstKept instanceof Element)) return;

  let parent = firstKept.parentElement;
  if (!(parent instanceof Element)) return;

  let cursor = firstKept.previousElementSibling;
  let safety = 0;

  while (cursor && safety < 20) {
    const prev = cursor.previousElementSibling;
    const isTurn = cursor.matches?.('section[data-testid^="conversation-turn-"][data-turn][data-turn-id]');
    if (isTurn) break;
    ...
    const likelyOrphan =
        ...
        text.includes("思考") ||
        text.includes("秒") ||
        cursor.querySelector?.('[data-testid="copy-turn-action-button"]') ||
        cursor.querySelector?.('button[aria-label*="复制"]') ||
        cursor.querySelector?.('button[aria-label*="喜欢"]') ||
        cursor.querySelector?.('button[aria-label*="不喜欢"]');

    if (likelyOrphan) cursor.remove();
    cursor = prev;
    safety += 1;
  }
}
...
function getRuleProbe(rule) {
  ...
  if (!(listEl instanceof Element)) return { ok: false, reason: "missing-list" };

  const grouping = buildRoundGroups(items, rule);
...
function findPresetRule() {
  ...
  for (const candidate of candidates) {
    try {
      ...
      const score =
          probe.totalCount * 100 +
          probe.items.length * 5 +
          ((probe.roleStats.user || 0) > 0 ? 30 : 0) +
          ((probe.roleStats.assistant || 0) > 0 ? 30 : 0) +
          (candidate.groupingStrategy ? 40 : 0);
  ...
  if (!best) return { ok: false, platform, error: "未命中站点预设" };
...
function applyRule(rule) {
  ...
  try {
    const { listEl, items } = getMatchedItems(rule);
    if (!listEl) return { ok: false, error: "未找到对话容器" };
    ...
    if (trimmedCount > 0) {
      ...
      cleanupChatGptOrphans(rule, grouping);
    }
    ...
    if (rule?.enabled) ensureBootstrapObserver();
...
function ensureObserver(listEl) {
  disconnectObserver();
  if (!currentRule?.enabled || !currentRule?.autoTrim || !(listEl instanceof Element)) return;
...
function scoreTurnCandidate(itemEl) {
  ...
  const similarSiblings = siblings.filter((sibling) => {
    ...
    return (
        sibling.tagName === itemEl.tagName &&
        getStableClassNames(sibling)[0] &&
        getStableClassNames(sibling)[0] === getStableClassNames(itemEl)[0]
    );
  });
  ...
  const avgText =
      similarSiblings.reduce((sum, el) => sum + Math.min(getTextLength(el), 600), 0) / similarSiblings.length;
  ...
  return (
      similarSiblings.length * 40 +
      avgText * 0.35 +
      Math.min(rect.height, 500) * 0.25 +
      scrollBonus * 20 +
      tagBonus * 10 +
      containsRoundBonus -
      directMessagePenalty
  );
}
...
function startPicker() {
  ...
  tip.textContent = "请选择一整条对话框，Esc 取消";
  ...
  const onMove = (event) => {
    ...
    hoveredCandidate = {
      ...inferred,
      element: document.querySelector(inferred.pickedSelector) || el,
    };
  ...
  const onKeyDown = (event) => {
    if (event.key === "Escape") destroyPicker();
  };
    ...
    try {
      ...
      if (message?.type === "getPlatformInfo") {
        ...
        sendResponse({
          ...
          platformKey: platform.key,
          platformLabel: platform.label,
          presetCount: platform.rules.length,
        });
        return;
      }
...
window.addEventListener("pageshow", () => {
  if (currentRule?.enabled) applyRule(currentRule);
});
