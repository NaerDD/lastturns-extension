...
ChatCut 是一个面向长对话场景的浏览器扩展。  
当你在各类大语言模型网页里连续聊天很多轮之后，页面中的 DOM 会越来越多，浏览器可能出现明显卡顿、滚动掉帧、输入延迟升高等问题。
...
ChatCut 会在当前网页中识别“对话轮次”对应的 DOM 结构。  
...
“清除站点规则”会删除当前网站保存的识别规则。  
当网站结构变化、之前手动选择不准确，或者你想重新识别当前站点时，可以使用这个功能。

它不会删除你的聊天记录，也不会清空账号历史。

---

### 隐私说明

ChatCut 默认不上传聊天内容，不读取账号信息，也不会将对话发送到外部服务器。
...
**Repository:** [NaerDD/ChatCut-extension](https://github.com/NaerDD/ChatCut-extension)
...
<div align="center">
  <img src="assets/wechat-pay.png" alt="WeChat Pay" width="180" />
  <img src="assets/alipay-pay.png" alt="Alipay" width="180" />
</div>
...
ChatCut is a lightweight browser extension designed for long AI chat sessions.

After many turns of conversation, large language model websites often accumulate a huge amount of DOM nodes, which can cause severe lag, slow scrolling, input delay, and high memory usage.
...
ChatCut identifies DOM structures corresponding to conversation turns on the current page.
...
2. Click the ChatCut icon in the browser toolbar
...
3. ChatCut will save the site rule and re-apply cleanup
...
This is useful when the site layout has changed, a previous manual selection was inaccurate, or you want ChatCut to re-detect the chat structure from scratch.

It does not delete your chat history or remove conversations from your account.

---

### Privacy

ChatCut does not upload your chat content, does not read your account information, and does not send conversation data to any external server.
...
ChatCut performs page-level cleanup by detecting website structures.
...
Compatibility and detection accuracy will continue to improve.